...
procedure leerVector(var v: vector; var dimL: integer);
	...
	begin
		...
		while (v[i].cod_proj <> fin) and (dimL <= dimF) do begin // Terminar si el código ingresado es -1 o la dim log. = dim fis.
			leerProyecto(v[i]);
			i:= i + 1;
			write('Código del proyecto: '); readln(v[i].cod_proj);
		end;
...
procedure leerArquitectosProyecto(c: codigo);
	var
		i:= integer;
	begin
		for i:= 1 to 1000 do begin
...
begin
	...
	writeln('El codigo del proyecto con menor monto invertido: ', cMontoMin);
end.
