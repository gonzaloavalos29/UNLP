...
procedure empleadoApeONom(var arc: archivo);
var
	texto: string[20];
	e: empleado;
begin
...
begin
	reset(arc);
	write('Lista de empleados mayores a 70 anios, prontos a jubilarse: ');
	while (not EOF(arc)) do begin
		read(arc, e);
		if (e.edad > 70) then
			imprimirEmpleado(e);
...
begin
	...
	leerEmpleado(e);
	while (e.apellido <> 'fin') do begin
		if (not(controlUnicidad(arc, e.numero))) then begin
...
begin
	...
	writeln('OPCION 8: Salir del menu y terminar la ejecución del programa');
	writeln();
	readln(opcion);
	while (opcion <> 8) do begin
		case opcion of 
			1: empleadoApeONom(arc);
