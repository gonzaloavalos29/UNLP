	...
	while (a.codigo <> valorAlto) do begin
		if (a.codigo < 0) then begin
			...
			if (filepos(arc)-1 <> 0) then
				while (a.codigo < 0) do begin // puede pasar que los ultimos también se tengan que borrar, por lo que ya los voy eliminando, una vez que encuentro
				// uno que no se tenga que borrar, salgo del while
					seek(arc, filesize(arc)-1);
					truncate(arc);
					seek(arc, filesize(arc)-1);
					read(arc, a);
