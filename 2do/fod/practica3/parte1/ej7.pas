...
begin
	...
	if (a.codigo <> -1) then begin
		...
		writeln('Ingrese la zona geográfica de la especie');
		readln(a.zona);
	end;
end;
	...
	while (a.codigo <> valoralto) do begin
		if (a.codigo < 0) then begin
			...
			if (filepos(arc)-1 <> 0) then
				while (a.codigo < 0) do begin // puede pasar que los ultimos tambien se tengan que borrar, por ende ya los voy eliminando,
