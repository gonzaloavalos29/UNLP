...
begin
	...
	while (info.codigo <> valorAlto) do begin
		...
		cantSuc:= 0;
		while (info.codigo = sucActual) do begin
			writeln(txt, 'Identificacion del autor: ', info.idAutor);
			autorActual:= info.idAutor;
			cantAutor:= 0;
			while ((info.codigo = sucActual) and (info.idAutor = autorActual)) do begin
				isbnActual:= info.isbn;
				cantISBN:= 0;
				while ((info.codigo = sucActual) and (info.idAutor = autorActual) and (info.isbn = isbnActual)) do begin
					cantISBN:= cantISBN + 1;
					leer(a, info);
				end;
