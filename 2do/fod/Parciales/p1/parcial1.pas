...
begin
	if (r1.codigoProducto < r2.codigoProducto) then begin
		min:= r1;
		leer(det1, r1);
	end else if (r2.codigoProducto < r3.codigoProducto) then begin
		min:= r2;
		leer(det2, r2);
	end else begin
		min:= r3;
...
begin
	...
	while (min.codigoProducto <> valorAlto) do begin
		read(mae, infoM);
		while (infoM.codigoProducto = min.codigoProducto) do begin
			if (min.cantPedida > infoM.stockActual) then begin
