program parcial5;
type
	infoMaestro = record // información sobre prendas de ropa
		codigo: integer;
		descripcion: string;
		colores: string;
		tipo: string;
...
begin
	...
	while (not eof(arc)) do begin
		read(arc, a);
		if (a.stock < 0) then begin
			pos:= filepos(arc)-1;
			seek(arc, filesize(arc)-1);
			read(arc, a);
			if (filepos(arc)-1 <> 0) then
				while (a.stock < 0) do begin // puede pasar que los ultimos tambien se tengan que borrar, por ende ya los voy eliminando,
				// una vez que encuentre uno que no se tenga que borrar salgo del while
					seek(arc, filesize(arc)-1);
					truncate(arc);
					seek(arc, filesize(arc)-1);
					read(arc, a);
...
begin
	...
	while (infoMae.codigo <> -1) do begin
