...
public class Capicua {
	...
	public static boolean esPalindromo(List<Integer> lista) {
		if (lista.isEmpty() || lista.size() == 1) return true;
			...
			if (!lista.get(0).equals(lista.get(lista.size()-1))) return false; // Si los primeros y los últimos elementos no coinciden, no es palíndromo
			else
				return esPalindromo(lista.subList(1, lista.size()-1)); // llama recursivamente a esPalindromo con una subLista sin los primeros y últimos elementos
	}
