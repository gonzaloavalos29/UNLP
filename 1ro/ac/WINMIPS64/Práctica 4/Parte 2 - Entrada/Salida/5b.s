...
LBU         $t1, Y($zero)                       # carga en $t1 la coordenada Y ____________
...
SB          $t2, 5($s1)                         # almaceno lo que hay en $t2 en DATA(X) ______________
DADDI       $t3, $zero, 5                       # funcion 5
SD          $t3, 0($s0)                         # mando a control 5
DADDI       $t2, $t2, 1                         # a la coordenada X le suma 1
BNE         $t4, $t2, LOOP                      # si $t4 = $t2 salta
DADDI       $t2, $zero, 0                       # hago que vuelva a la coordenada X = 0 ______________
