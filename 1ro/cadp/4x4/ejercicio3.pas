{ Se dispone de datos de empleados de una empresa multinacional. De cada empleado se conoce su código de empleado (entre 1 y 2000),
* el código de país donde trabaja (entre 1 y 25), la antigüedad en años y el sueldo bruto en dólares. La información se encuentra
...
begin
	...
	begin
		...
		while (pos < dimL) and (pais_actual = empleados[pos].pais) do begin
			e:= empleados[pos];
			...
			if (cumple(e)) then
				cantEmp:= cantEmp + 1;
			pos:= pos + 1;
		end; // fin del while interior
		if (cantPaisActual > cantPaisMax) then
		begin
			cantPaisMax:= cantPaisActual;
			paisMax:= pais_actual;
		end;
		writeln('El codigo del empleado con mejor sueldo en el pais ', pais_actual, ' es ', mejorSueldo.codigo);
	end; // fin del while exterior
end;

var
	paisMax: codigos_paises;
	cantEmpleado: integer;
	empleados: vector_empleados;
	dimL: codigos_empleados;
begin
	cargarEmpleados(empleados, dimL) // SE DISPONE
	cantEmpleados:= 0;
	procesarDatos(empleados, dimL, cantEmpleados, paisMax);
	writeln('El pais con mayor cantidad de empleados es: ', paisMax);
	writeln('La cant. de empleados con mas de 10 anios de antiguedad cuyo sueldo bruto no alcance los 1500 dolares es ',
	cantEmpleados);
end.
