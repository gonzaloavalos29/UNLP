...
type
	...
	cliente = record
		...
		apellido: string[10];
		nombre: string[10];
		codigo_poliza: r_ppoliza; // POLIZA CONTRATADA (DEL 1 AL 6);
		monto: real;
	end;
	nodo = record
		data: cliente;
	...
	// La empresa dispone de una tabla donde guarda un valor que representa un monto adicional que el cliente debe abonar en la
	// liquidación mensual de su seguro, de acuerdo al código de póliza que tiene contratada.
...
procedure procesarLista(l: lista); // Este procedimiento usa el vector de los codigos poliza.
begin
	while (l <> nil) do begin
		...
		if (contarDigitosNueve(l^.data.dni) >= 2) then begin
			write(' / ' + 'Su DNI contiene al menos dos digitos 9');
		end;
...
var
	l: lista;
	v1: v_poliza; // SE DISPONE DE LOS VALORES DE LIQUIDACION POR CODIGO DE POLIZA
begin
	l:= nil;
	cargarLista(l);
	// SE DISPONE
	procesarLista(l);
	eliminarNodo(l, 120); // SEGURO EXISTE EL CODIGO
end.
