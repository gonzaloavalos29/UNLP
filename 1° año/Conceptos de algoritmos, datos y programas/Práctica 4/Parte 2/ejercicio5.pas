...
function cuantosSuperanMonto(v: vector; monto: real): integer;
	...
	begin
		aux:= 0;
		for i:= 1 to dimF do begin // Revisar todos los elementos del vector para ver cuántos superan el monto
...
procedure cargarVector(var v: vector);
	...
	begin
		for i:= 1 to dimF do begin
			leerCliente(v[i]); // Asignarle el espacio del vector actual un registro
			writeln;
		end;
	end;
...
procedure leerCiudades(ciudades: vCod); // Punto C
	...
	begin
		montoMax:= -1;
		codMax:= 0;
		for i:= 1 to 10 do begin // Ir sacando el monto mayor por la fuerza bruta
			...
			writeln('El codigo de la ', i, '° ciudad con el mayor monto es: ', codMax);
...
procedure procesarVector(var v: vector; var vContMes: vContMes; var vContAno: vContAno; var vCat: vCat; var vCod: vCod; var anoMax: integer; var cantEsp: integer);
	...
	begin
		...
		for i:= 1 to dimF do begin
			...
			vContAno[v[i].fecha.ano]:= vContAno[v[i].fecha.mes] + 1;
