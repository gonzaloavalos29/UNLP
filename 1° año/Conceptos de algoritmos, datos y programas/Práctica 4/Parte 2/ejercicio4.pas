...
type
	...
	vector = array[1..dimF] of alumno; // vector de alumnos
...
procedure cargarVector(var v: vector; var dimL: integer);
	...
	begin
		i:= 0;
		seguir:= true;
		while (seguir) and (i <= dimF) do begin // Mientras la dimL no supere la dimF...
			i:= i + 1;
			dimL:= dimL + 1;
			leerAlumno(v[i]);
...
procedure insertarAlumno(var v: vector; var dimL: integer; a: alumno); // Punto B
	begin
		if (dimL < dimF) then begin // verifico si la dimL puede seguir aceptando gente o no
...
procedure eliminarAlumnoA(var v: vector; var dimL: integer; posAlumno: integer); // Punto C
	var
		i: integer;
	begin
		for i:= posAlumno to dimL do begin //Hacer que recorra desde la pos ingresada hasta el límite lógico
			v[i]:= v[i+1]
		end;
		dimL:= dimL - 1;
	end;
