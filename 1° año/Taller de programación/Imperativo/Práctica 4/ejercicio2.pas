...
procedure InformarExistenciaNombreSocio(a: arbol);
	...
	function encontrarNombre(a: arbol; numSocio: integer): boolean;
	begin
		if (a = nil) then 
			encontrarNombre:= false
		else 
			encontrarNombre(a^.HI, numSocio);
			if (a^.dato.numero = numSocio) then
				encontrarNombre:= true;
			encontrarNombre(a^.HD, numSocio);
	end;
var
	numSocio: integer;
	encontrado: boolean;
begin
	numSocio:= 10;
	encontrado:= encontrarNombre(a, numSocio);
	if encontrado then
		writeln('El socio: ', numSocio, ' fue encontrado.')
	else
		writeln('El socio: ', numSocio, ' no fue encontrado.');
end;

procedure InformarCantidadSocios(a: arbol);

	function contarSocios(a: arbol; extremoInf, extremoSup: integer): integer;
	begin
		if (a = nil) then
			contarSocios:= 0
		else
			if (a^.dato.numero < extremoInf) then
				contarSocios(a^.HD, extremoInf, extremoSup)
			else if (a^.dato.numero > extremoSup) then 
				contarSocios(a^.HI, extremoInf, extremoSup)
			else
				contarSocios(a^.HI, extremoInf, extremoSup);
				contarSocios:= contarSocios + 1;
				contarSocios(a^.HD, extremoInf, extremoSup);
	end;
begin
	writeln('La cantidad de socios es: ', contarSocios(a, 5, 10));
end;
...
Begin
  ...
  InformarSociosOrdenDecreciente (a); //COMPLETAR 1)
  InformarNumeroSocioConMasEdad (a);
  AumentarEdadNumeroImpar (a);
  InformarExistenciaNombreSocio (a); // COMPLETAR 2)
