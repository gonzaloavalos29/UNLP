...
begin
	write('Codigo de la materia: '); readln(m.codigo);
...
begin
	if (a = nil) then begin
		...
		if (alu.legajo <= a^.data.legajo) then
...
begin
	while (l <> nil) do begin
		writeln('Codigo de la materia: ', l^.data.codigo, ' / Nota sacada: ', l^.data.nota);
		l:= l^.sig;
	end;
end;
...
procedure puntoE(a: arbol; var cant: integer); // No hay método para buscar más eficientemente así q toca recorrerla todaaa
begin
	if (a <> nil) then begin // Siempre verificar primero que el árbol no se encuentre vacío
...
procedure puntoF(a: arbol; var legajoMax2: integer; var promMax: real); // Como no hay manera de recorrer la lista de manera más eficiente, toca recorrerla toda
begin
	if (a <> nil) then begin
