...
begin
	...
	if (com.codigo <> codigo_corte) then begin
		...
		com.monto:= (Random(100) / 5); 
		writeln('Monto: ', com.monto:2:2);
	end;
end;

procedure inicializarVector(var v: vectorMes); // Inicializar vector de meses
var
	i: integer;
begin
	for i:= 1 to dimF do
...
begin
	if (arb = nil) then begin // Comprobar si el árbol está vacío
		new(arb);
		arb^.dato.codigo:= com.codigo; // Asignarle el código pasado en el módulo al árbol
		inicializarVector(arb^.dato.meses);
		arb^.dato.meses[com.mes]:= com.monto;
		arb^.HI:= nil;
		arb^.HD:= nil;
	end else begin // SI no lo está...
		if (com.codigo < arb^.dato.codigo) then begin
			agregarArbol(arb^.HI, com);
		end else if (com.codigo > arb^.dato.codigo) then begin
...
procedure puntoB(arb: arbol; cliente: integer; var mesMax: integer);
	//--- FUNCIÓN ADENTRO
	function mesMayor(v: vectorMes): integer;
	var
		i, mesMax: integer;
		montoMax: real;
	begin
		...
		for i:= 1 to dimF do begin
			if (v[i] > montoMax) then begin // SI el gasto del elemento actual del vector es mayor a la variable
				montoMax:= v[i];
				mesMax:= i;
			end;
		end;
...
// PUNTO C
procedure puntoC(arb: arbol; numMes: integer; var cantClientes: integer);
begin
	if (arb <> nil) then begin
...
procedure imprimirVectorMes(v: vectorMes);
var
	i: integer;
begin
	for i:= 1 to dimF do begin
		writeln('Monto del mes: ', i, ' es ', v[i]:2:2);
	end;
end;
...
VAR
	a: arbol;
	cantClientes, numInput, mesMax: integer;
BEGIN
	a:= nil; // inicializar árbol
	mesMax:= 0;
	cantClientes:= 0;
	Randomize;
	cargarArbol(a);
	...
	// PUNTO B
	writeln('Ingrese un codigo de cliente, '); readln(numInput);
	puntoB(a, numInput, mesMax);
	writeln('El mes con mayor monto del cliente es: ', mesMax);
	
	// PUNTO C
	write('Ingrese un mes: '); readln(numInput);
	puntoC(a, numInput, cantClientes);
	writeln('Cantidad de clientes que no tienen gastos en el mes ', numInput, ': ', cantClientes);
END.
