...
begin
	if (a <> nil) then begin // El árbol no está vacío
		if (a^.data.legajo < valor) then begin
			...
			puntoB(a^.HD, valor); // Acceder al nodo derecho de manera recursiva
		end else begin // Si el valor es mayor o igual, ->
...
procedure puntoD(a: arbol; var DNI: integer); // No queda otra que recorrer toda la lista
begin
	if (a <> nil) then begin
...
procedure puntoE(a: arbol; var cant: integer); // No hay método para buscar más eficientemente así q toca recorrerla toda
begin
	if (a <> nil) then begin // Siempre verifico primero que el árbol no se encuentre vacío
...
procedure puntoF(a: arbol; var legajoMax2: integer; var promMax: real); // Como no hay manera de recorrer la lista de manera más eficiente, toca recorrerla toda otra vez
begin
	if (a <> nil) then begin
...
procedure puntoG(a: arbol; valor: integer; var listaEspecial: lista2); // Again, no hay búsqueda optimizada
var
	a2: alumno2;
begin
	if (a <> nil) then begin
...
begin
	while (listaEsp <> nil) do begin
		write('Legajo: ', listaEsp^.data.legajo, ' / promedio: ', listaEsp^.data.promedio:2:2);
