...
begin
	write('Codigo del producto: '); readln(ven.codigo);
...
begin
	...
	if (a <> nil) then begin // Si el árbol no está vacío...
		if (codigo < a^.data.codigo) then begin // Si el código es menor que el del nodo actual del árbol...
			cant:= puntoB(a^.HI, codigo); // Ir al hijo izquierdo
		end else if (codigo > a^.data.codigo) then begin // Si el código es mayor que el del nodo actual del árbol...
			cant:= puntoB(a^.HD, codigo); // Ir al hijo derecho
		end else begin // Sino (o sea si es igual) ->
			cant:= a^.data.unidades; // Asignarle a cant las unidades del nodo actual
			cant:= cant + puntosB(a^.HI, codigo); // Y sumarle las que hayan del lado izquierdo
...
begin
	cant:= 0; // Inicializar variable en 0
	if (a2 <> nil) then begin // Si el árbol no está vacío...
		if (codigo < a2^.data.codigo) then begin // Si el código es menor que el del nodo actual del árbol...
			cant:= puntoC(a2^.HI, codigo); // Ir al hijo izquierdo
		end else if (codigo > a2^.data.codigo) then begin // Si el código es mayor que el del nodo actual del árbol...
			cant:= puntoC(a2^.HD, codigo); // Ir al hijo derecho
		end else begin // Sino (o sea, si es igual) ->
			cant:= a2^.data.unidades; // Asignarle a cant las unidades del nodo actual
		end;
	end;
	puntoC = cant; // Retornar la cantidad de unidades vendidas
end;
...
begin
	...
	cargarArbol2(a);
	mirarArbol(a);
	writeln('El codigo ', 10, ' tuvo ', puntoB(a, 10);
	writeln('El codigo ', 5, ' tuvo ', puntoC(a2, 5);
END.
